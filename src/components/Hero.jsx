...
import { Play, Target, Heart, Activity } from 'lucide-react';
...
const Hero = ({ initialNiche = 'motivation' }) => {
    ...
    const handleNicheSwitch = (n) => {
        setCurrentNiche(n);
        trackEvent(ANALYTICS_EVENTS.NICHE_SWITCH, { niche_id: n.id });
    };

    const handleDownloadClick = () => {
        trackEvent(ANALYTICS_EVENTS.DOWNLOAD_CLICK, { location: 'hero' });
    };
    ...
    return (
        <section className="relative min-h-screen flex items-center pt-24 pb-12 overflow-hidden">
            <motion.div
                layout
                className="absolute top-1/4 -left-20 w-96 h-96 rounded-full mix-blend-screen filter blur-[120px] animate-float opacity-30"
                style={{ backgroundColor: `var(--color-${currentNiche.accent})` }}
            ></motion.div>
            <div className="absolute bottom-1/4 -right-20 w-96 h-96 bg-accent/20 rounded-full mix-blend-screen filter blur-[100px] animate-float opacity-20" style={{ animationDelay: '2s' }}></div>
            ...
            <div className="max-w-7xl mx-auto px-6 w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
                <div className="text-center lg:text-left flex flex-col justify-center">
                    <motion.div
                        ...
                        className="inline-flex items-center gap-2 mb-6 px-4 py-1.5 rounded-full border border-primary/40 bg-primary/10 text-sm font-medium backdrop-blur-sm mx-auto lg:mx-0 shadow-[0_0_15px_rgba(13,92,186,0.2)]"
                    >
                    ...
                    <div className="flex justify-center lg:justify-start gap-2 mb-8 p-1.5 rounded-2xl bg-surface/50 border border-white/5 w-fit mx-auto lg:mx-0 backdrop-blur-md">
                        {Object.values(NICHES).map((n) => (
                            <button
                                key={n.id}
                                onClick={() => handleNicheSwitch(n)}
                                className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-300 flex items-center gap-2 ${currentNiche.id === n.id
                                    ? `bg-primary/20 text-white border border-white/10 shadow-lg`
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            <h1 className="font-display font-bold text-5xl md:text-6xl lg:text-7xl leading-tight mb-6 tracking-tight">
                                {currentNiche.headlineFirst} <br className="hidden lg:block" />
                                <span className="gradient-text">{currentNiche.headlineSecond}</span>
                            </h1>
                ...
                <div className="relative flex justify-center lg:justify-end">
                    <motion.div
                        layout
                        className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[300px] h-[500px] filter blur-[80px] rounded-full opacity-30"
                        style={{ backgroundColor: `var(--color-${currentNiche.accent})` }}
                    ></motion.div>
                    ...
                    <div className="relative w-[300px] h-[600px] bg-[#0a0a0f] border-[10px] border-[#1d1d2b] rounded-[2.5rem] shadow-2xl overflow-hidden shadow-[0_0_50px_rgba(0,229,255,0.15)] flex flex-col group transform transition-transform duration-700 hover:-translate-y-2">
                        ...
                        <div className="flex-1 bg-surface relative overflow-hidden group/screen cursor-pointer" onClick={togglePlay}>
                            <div className={`absolute inset-0 bg-linear-to-b from-primary/20 via-background to-background animate-pulse-slow ${isPlaying ? 'opacity-0' : 'opacity-100'} transition-opacity duration-500 z-0`}></div>

                            <AnimatePresence mode="wait">
                                <motion.video
                                    key={currentNiche.video}
                                    ref={videoRef}
                                    className="absolute inset-0 w-full h-full object-cover transition-opacity duration-700"
                                    src={currentNiche.video}
                                    loop
                                    playsInline
                                    muted
                                    initial={{ opacity: 0 }}
                            ...
                            <div className="absolute bottom-6 left-4 right-4 z-20 flex justify-between items-end pointer-events-none">
                                <div className="flex-1 pr-2">
                                    <motion.div
                                        ...
                                        animate={{ opacity: 1, y: 0 }}
                                        className="font-bold text-lg text-white mb-1 shadow-sm drop-shadow-lg leading-tight"
                                    >
                                        {currentNiche.title}
                                    </motion.div>
                                    <div className="text-sm text-white/90 mb-4 shadow-sm drop-shadow-lg leading-snug">AI-curated tailored precisely for you. 🚀</div>

                                    <div className="flex items-center gap-2">
                                        <div className="w-8 h-8 rounded-full bg-linear-to-br from-primary to-accent p-[1px] shadow-[0_0_10px_rgba(0,229,255,0.5)] shrink-0">
                                            <div className="w-full h-full bg-surface rounded-full flex items-center justify-center overflow-hidden">
                                                <img src="https://i.pravatar.cc/100?img=1" alt="UBIRT.AI Creator Avatar" className="w-full h-full object-cover" />
                                            </div>
                                        </div>
